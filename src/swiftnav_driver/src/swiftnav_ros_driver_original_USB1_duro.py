...
pub_enu_point = rospy.Publisher(
    rospy.get_name() + "/enu_pose_best_fix", Point, queue_size=10
)
pub_enu = rospy.Publisher(rospy.get_name() + "/enu_pose", gps_loc, queue_size=10)
pub_ned = rospy.Publisher(
    rospy.get_name() + "/ned_pose_best_fix", gps_loc, queue_size=10
)
pub_imu_raw = rospy.Publisher(rospy.get_name() + "/imu_raw", imu, queue_size=10)
pub_imu_scaled = rospy.Publisher(
    rospy.get_name() + "/imu_scaled", imu_scaled, queue_size=10
)
...
def cb_SBP_MSG_POS_LLH(msg_llh, **source):
    """
    Callback function for SBP_MSG_POS_LLH messages.
    Publishes the received GPS position in LLH format.

    Args:
        msg_llh: The SBP_MSG_POS_LLH message object.
...
def cb_SBP_MSG_BASELINE_NED(msg_ned, **source):
    """
    Callback function for SBP_MSG_BASELINE_NED messages.
    Publishes the received baseline in NED format.

    Args:
        msg_ned: The SBP_MSG_BASELINE_NED message object.
        source: Additional source information.
...
def cb_SBP_MSG_IMU_RAW(msg_imu_raw, **source):
    """
    Callback function for SBP_MSG_IMU_RAW messages.
    Publishes the received raw IMU data.

    Args:
        msg_imu_raw: The SBP_MSG_IMU_RAW message object.
        source: Additional source information.
...
def cb_SBP_MSG_IMU_AUX(msg_imu_aux, **source):
    """
    Callback function for SBP_MSG_IMU_AUX messages.
    Processes the received IMU auxiliary data.

    Args:
        msg_imu_aux: The SBP_MSG_IMU_AUX message object.
        source: Additional source information.
